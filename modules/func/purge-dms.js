...
module.exports.run = async (client, msg, args, config) => {
    ...
    while(array.length > i) {
       ...
       if (filtered.size === 0) {
           console.log('[+][MODULE] -> No messages found in ' + client.channels.get(array[i]).recipient.username + ', moving onto the next one.')
           i++
           continue;
       }
       ...
       if (under100.size < 100) {
           ...
           console.log('[+][MODULE] -> deleted ' + filtered.size + ' message(s) in ' + client.channels.get(array[i]).recipient.username)
           i++;
           continue;
       }
       ...
       for(let d = 0; d < messages.length; d++) {
           ...
           console.log('[+][MODULE] -> deleted 1 message in ' +  client.channels.get(array[i]).recipient.username)
       }
       let messagegetarray = messageget.last().id
       messageID = messagegetarray
       continue;
    } catch(err) {
        console.log('[+][MODULE] -> System message found, sending next iteration.')
        i++;
    }
}


console.log('[+][MODULE] -> Finished deleting messages')
}
...
module.exports.help = {
    ...
    example:";purge-dms",
    group: "func",
    desc:"Deletes all your messages in every DM you have open. <RISK:LOW>"
}
